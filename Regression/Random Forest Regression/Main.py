...
dataset = pd.read_csv(r'D:\Learning\Udemy ML\MLPracticeProjects\Regression\Random Forest Regression\Position_Salaries.csv')
x = dataset.iloc[:, 1:-1].values
y = dataset.iloc[:, -1].values

# training random forest regression model
from sklearn.ensemble import RandomForestRegressor
regressor = RandomForestRegressor(n_estimators = 10, random_state = 0) # number of trees to be considered
regressor.fit(x, y)
...
# graphs gives more steps as there more trees being considered and there more feature extraction