...
dataset = pd.read_csv(r'D:\Learning\Udemy ML\MLPracticeProjects\Classification\Naive Bayes Intuition\Social_Network_Ads.csv')
...
plt.title('Naive Bayes Intuition (Training Set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)) : 
    plt.scatter(x_set[y_set == j, 0], x_set[y_set == j, 1], c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Naive Bayes Intuition (Test Set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()